...
class TestUtils:
    ...
    def test_default_maturity_is_weekday(self):
        date = get_default_maturity_date()
        dow = date.dayOfWeek()
        assert 1 <= dow <= 5, f"La date de maturité tombe un weekend (jour {dow})"

    def test_default_maturity_roughly_60_days(self):
        from PySide6.QtCore import QDate
        today = QDate.currentDate()
        date = get_default_maturity_date()
        diff = today.daysTo(date)
        # 60 jours + au maximum 2 jours d'ajustement weekend
        assert 60 <= diff <= 62
